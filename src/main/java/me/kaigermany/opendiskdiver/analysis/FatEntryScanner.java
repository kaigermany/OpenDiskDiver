...
public class FatEntryScanner implements Scanner {
	...
	private static boolean isHerachieDirEntry(byte[] block, int pos){
		if(block[pos] != '.') return false;
		if(block[pos + 1] != '.' && block[pos + 1] != ' ') return false;
		
		for(int i=2; i<11; i++){
			if(block[pos + i] != ' ') return false;
		}

		if(block[pos + 11] != 0x10) return false;//not a dir?
		/*
		if(block[pos + 21] != 0 || block[pos + 22] != 0) return false;//must be 0
		*/
		return true;
	}
...
/*
		final int[] VALID_ATTRIBUTES = { 0x01, 0x10, 0x20, 0x30, 0x08, 0x40, 0x0F };
		for (int validAttribute : VALID_ATTRIBUTES) {
			if (attr == validAttribute) {
				return true;
			}
		}
		return false;
*/
	...
	private static boolean testFor8Dot3Name(byte[] buffer, int offset, boolean disallowUpper128chars) {
